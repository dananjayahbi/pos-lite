...
export default function StaffDetailPage({
  ...
  return (
    <div className="space-y-6">
      ...
      {activeTab === 'timeclock' && (
        <TimeClockHistory staffId={staffId} sessionRole={sessionRole} />
      )}
    </div>
  );
}
...
interface TimeClockSummaryData {
...
function TimeClockHistory({ staffId, sessionRole }: { staffId: string; sessionRole: string }) {
  const queryClient = useQueryClient();
  const [page, setPage] = useState(1);
  const [closeDialogOpen, setCloseDialogOpen] = useState(false);
  const [closingRecordId, setClosingRecordId] = useState<string | undefined>(undefined);
  const pageSize = 10;
  ...
  const [summary, setSummary] = useState<TimeClockSummaryData>({ hoursThisWeek: 0, hoursThisMonth: 0 });

  useEffect(() => {
    // Compute from records or use a dedicated call
    if (data?.data?.records) {
      ...
      for (const r of data.data.records) {
        const clockIn = new Date(r.clockedInAt);
        const clockOut = r.clockedOutAt ? new Date(r.clockedOutAt) : now;
  ...
  const closeSessionMutation = useMutation({
    mutationFn: async () => {
      // Manager closing an open session for another user — use clock-out API
      ...
      if (!res.ok) {
        const err = await res.json();
        throw new Error(err?.error?.message ?? 'Failed to close session');
      }
      return res.json();
    },
    onSuccess: () => {
      toast.success('Time clock session closed');
      setCloseDialogOpen(false);
      setClosingRecordId(undefined);
      queryClient.invalidateQueries({ queryKey: ['timeclock', staffId] });
      queryClient.invalidateQueries({ queryKey: ['staff-detail', staffId] });
    },
    onError: (err) => {
      toast.error(err.message);
  ...
  const isManager = ['MANAGER', 'OWNER'].includes(sessionRole);

  return (
    <div className="space-y-4">
      <Card>
        <CardHeader>
          <CardTitle className="text-lg font-display text-espresso">Time Clock</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          {/* Summary */}
          <div className="flex flex-wrap gap-3">
          ...
          {isLoading ? (
            ...
            <p className="text-sand text-sm py-4 text-center">No time clock records found.</p>
          ) : (
            <>
              <Table>
                <TableHeader>
                ...
                <TableBody>
                  {records.records.map((r) => (
                    <TableRow key={r.id}>
                      <TableCell className="text-sm">
                        {new Date(r.clockedInAt).toLocaleDateString('en-LK')}
                      </TableCell>
                      <TableCell className="text-sm font-mono">
                        {new Date(r.clockedInAt).toLocaleTimeString('en-LK', {
                          hour: '2-digit',
                          minute: '2-digit',
                        })}
                      </TableCell>
                      <TableCell className="text-sm font-mono">
                        {r.clockedOutAt
                          ? new Date(r.clockedOutAt).toLocaleTimeString('en-LK', {
                              hour: '2-digit',
                              minute: '2-digit',
                            })
                          : '—'}
                      ...
                      <TableCell className="text-sm">
                        {r.durationMinutes !== null ? formatMinutes(r.durationMinutes) : '—'}
                      </TableCell>
                      <TableCell className="text-xs font-mono">
                        {r.shift ? r.shift.id.slice(0, 8) + '…' : '—'}
                      </TableCell>
                      <TableCell className="text-sm text-sand max-w-[120px] truncate">
                        {r.notes ?? '—'}
                      </TableCell>
                      <TableCell>
                        {r.clockedOutAt ? (
      ...
      <Dialog open={closeDialogOpen} onOpenChange={setCloseDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="font-display">Close Time Clock Session</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <p className="text-sm text-espresso">
              Are you sure you want to close this open time clock session?
            </p>
            <div className="flex justify-end gap-2">
